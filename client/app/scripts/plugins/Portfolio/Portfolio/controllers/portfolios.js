...
/**
 * @ngdoc function
 * @name Portfolios.controller:PortfoliosController
 * @description
 * # PortfoliosController
 * Controller of the Portfolios
 */
angular.module('getlancerApp.Portfolio')
    .controller('PortfoliosController', ['$scope', '$rootScope', '$location', '$window', '$filter', '$state', 'md5', 'PortfoliosFactory', 'flash', '$uibModal', '$uibModalStack', 'HelperFactory', 'SweetAlert', 'Upload', '$stateParams', 'PortfolioConstant', 'PortfolioSkillsFactory', 'Slug', 'PortfolioAutocompleteUsers', 'ConstUserRole', '$timeout','mePortfoliosFactory', '$cookies', function($scope, $rootScope, $location, $window, $filter, $state, md5, PortfoliosFactory, flash, $uibModal, $uibModalStack, HelperFactory, SweetAlert, Upload, $stateParams, PortfolioConstant, PortfolioSkillsFactory, Slug, PortfolioAutocompleteUsers, ConstUserRole, $timeout,mePortfoliosFactory, $cookies) {
        /* [ Function to set Page Header ] */
        function setRootHeader(pagename) {
            $rootScope.header = $rootScope.settings.SITE_NAME + ' | ' + $filter("translate") ("Portfolio")+' ' +'-' + ' ' +(pagename);
        }
        /* [ Create Slug ] */
        ...
        $rootScope.closemodel = function() {
             $uibModalStack.dismissAll();
             $scope.port_url = JSON.parse($cookies.get("portfolio_url"));
                             if ($scope.port_url ===  'dashboard') {
        ...
        $scope.lastpage = 2;
        $scope.loader = true;
        $scope.location_find = $location.path().split("/")[2];
        function getPortfolioData() {
            ...
            if ($state.params.skills !== null && $state.params.skills !== undefined) {
                params.skill = $state.params.skills;
                 $scope.NoRecordMessage = false;
            } else if ($scope.location_find === 'dashboard') {
                $scope.title_name = 'My Portfolios';
                $scope.NoRecordMessage = false;
            ...
            if ($scope.location_find === 'dashboard') {
                $cookies.put('portfolio_url',JSON.stringify('dashboard'));
                $scope.port_url = JSON.parse($cookies.get("portfolio_url"));
                $scope.NoRecordMessage = false;
             mePortfoliosFactory.get(params, function(response) {
                if ($scope.location_find !== 'dashboard') {
                $rootScope.header = $rootScope.settings.SITE_NAME + ' | ' + $filter("translate") ("Portfolios");
                 $scope.title_name = 'Portfolios';
                }
                $scope.scroll_flag = true;
                ...
                $scope.scroll_flag = true;
                  ...
                  $cookies.put('portfolio_url',JSON.stringify('Portfolio'));
                  $scope.port_url = JSON.parse($cookies.get("portfolio_url"));
		...
		$scope.page = $scope.currentpage;
		 $scope.pagination = function() {
				...
				if ($scope.enabled === true && $scope.scroll_flag === true) {
					if ($scope.currentpage <= $scope.lastpage) {
						if ($scope.firstVisit !== 0) {
							$scope.currentpage += 1;
                             $scope.back_to_top = true;
							$scope.scroll_flag = false;
						     getPortfolioData();	
                           $scope.scroll_load = false;							
						}
						$scope.firstVisit = 1;
					} else {
						$scope.enabled = false;
            ...
            $scope.backAgain = function() {
                    $("html, body").animate({ scrollTop: 0 }, "slow");//jshint ignore:line
                    return false;     
            }
        ...
        $scope.openPhotoModal = function(id, title, index, size, key) {
            ...
            $scope.modalInstance = $uibModal.open({
                ...
                controller: function($scope, $rootScope, photoid, photoindex, photoKey, photos, follow, redirect, $stateParams, $filter, md5, $location, $uibModalStack, $uibModal, $state, photoStatsFactory, PortfoliosFactory, userSettings, $timeout, HelperFactory) {
                    ...
                    $rootScope.nextvalue =false;
                    $scope.slide_change = function(option) {
                        $rootScope.nextvalue= true;
                        if (option === 'next') {
    	...
    	$(document).ready(function(){
            $(window).scroll(function(){
                if ($(document).scrollTop() > 300) {                    
                    ...
                    $('.scrolltop').css("right","30px");
                } else { 
                    $('.scrolltop').css("right","-40px");                    
                }
            });
